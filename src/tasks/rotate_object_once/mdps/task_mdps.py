...
def yaw_target_achieved(env, command_name: str = "object_pose") -> torch.Tensor:
    """Terminate when the active yaw target has been achieved."""
    command = env.command_manager.get_term(command_name)
    return command.metrics["yaw_target_active"].bool() & command.metrics[
        "trajectory_command_achieved"
    ].bool()
...
__all__ = ["anchor_object_z_axis_joint", "trajectory_yaw_tracking", "yaw_target_achieved"]
