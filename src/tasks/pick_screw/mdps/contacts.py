...
# ---------------------------------------------------------------------------
# Contact-sensing target registry (pick_screw)
# ---------------------------------------------------------------------------
...
# from it (see env_cfg.py), and the contact observations reference targets by
# *role* (object vs. external) resolved through this registry — never by a literal
# index.
#
...
# can brace/pivot against during a screw/insert: the receptacle (table top) and the
# table slab. Adding another external target is a one-line edit here plus a matching
# scene asset.
...
def external_indices() -> list[int]:
    """Filter indices of every non-object contact target (receptacle, table, ...).
