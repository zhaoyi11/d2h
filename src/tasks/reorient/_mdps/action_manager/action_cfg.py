...
@configclass
class EMARelativeJointPositionToLimitsActionCfg(JointPositionToLimitsActionCfg):
    """Configuration for EMA-smoothed relative joint position action with limit clamping."""

    class_type: type[ActionTerm] = EMARelativeJointPositionToLimitsAction
    """Class type."""

    alpha: float | dict[str, float] = 0.8
    """The weight for the moving average (float or dict of regex expressions)."""

    joint_lower_limit: list[float] | None = None
    joint_upper_limit: list[float] | None = None
    """Optional explicit lower and upper limits for target joint positions."""
