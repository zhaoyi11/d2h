...
The scripted reach->lift->move->align->approach->insert->hold peg trajectory that is specific to the
pick-insert task, composed from the task-agnostic primitives in :mod:`src.policy.high_level.utils`.
...
def build_pick_insert_object_pose_sequence(
    ...
    inserted pose, and holds there -- 7 segments: reach / lift / move / align / approach / insert /
    hold. The reach segment is typically 0-step (see :data:`DEFAULT_PICK_INSERT_SEGMENT_STEPS`), so it
    adds no waypoints and the object never moves during it. The keyframes are interpolated by the
    ...
    # object before it moves toward the receptacle.
