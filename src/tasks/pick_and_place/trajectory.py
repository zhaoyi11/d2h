...
def load_carry_trajectory(path=DEFAULT_TRAJECTORY, start_frame=320, end_frame=511, stride=32):
    ...
    # Keep the shared stage indices; zero deposit steps advances directly to release.
    segments = (0, 1, len(carry) - 1, 0, 1, 1)
    # Synthetic release/retreat targets have no source frame.
    source_frames = np.r_[frames[0], frames, np.full(2, -1)]
    return carry, source_frames, segments


def build_pick_and_place_trajectory(
    carry, current_pose_w, box_pose_w,
    box_target_offset=(0.0, 0.0, 0.10),
):
    """Return reach/grasp/carry/release/retreat poses in world coordinates.
    ...
    drop = box_pose_w[:3] + box_rotation.apply(box_target_offset)
    relative = carry[:, :3] - carry[0, :3]
    source = relative[-1]
    target = drop - current_pose_w[:3]
    if target[2] <= 0:
        raise ValueError("The drop target must be higher than the settled object.")
    ...
    return np.concatenate((current_pose_w[None], transported,
                           transported[[-1]], transported[[-1]]))
